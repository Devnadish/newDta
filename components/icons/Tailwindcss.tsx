import React from "react";

export function LineiconsTailwindcss(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={65}
      height={64}
      viewBox="0 0 65 64"
      {...props}
    >
      <path
        fill="#ffffff"
        fillRule="evenodd"
        d="M17.86 26q3-12 15-12c12 0 13.5 9 19.5 10.5q6 1.5 10.5-4.5q-3 12-15 12c-12 0-13.5-9-19.5-10.5q-6-1.5-10.5 4.5m-15 18q3-12 15-12c12 0 13.5 9 19.5 10.5q6 1.5 10.5-4.5q-3 12-15 12c-12 0-13.5-9-19.5-10.5q-6-1.5-10.5 4.5"
        clipRule="evenodd"
      ></path>
    </svg>
  );
}

export function TeenyiconsHtml5Outline(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={64}
      height={64}
      viewBox="0 0 15 15"
      {...props}
    >
      <path
        fill="#ffffff"
        d="M.5.5V0a.5.5 0 0 0-.498.542zm14 0l.498.042A.5.5 0 0 0 14.5 0zm-1 12l.158.474a.5.5 0 0 0 .34-.433zm-6 2l-.158.474a.5.5 0 0 0 .316 0zm-6-2l-.498.041a.5.5 0 0 0 .34.433zm3-9V3H4v.5zm0 3H4V7h.5zm6 0h.5V6h-.5zm0 3l.158.474L11 9.86V9.5zm-3 1l-.158.474l.158.053l.158-.053zm-3-1H4v.36l.342.114zM.5 1h14V0H.5zM14.002.458l-1 12l.996.083l1-12zm-.66 11.568l-6 2l.316.948l6-2zm-5.684 2l-6-2l-.316.948l6 2zm-5.66-1.567l-1-12l-.996.083l1 12zM11 3H4.5v1H11zm-7 .5v3h1v-3zM4.5 7h6V6h-6zm5.5-.5v3h1v-3zm.342 2.526l-3 1l.316.948l3-1zm-2.684 1l-3-1l-.316.948l3 1zM5 9.5V8H4v1.5z"
      ></path>
    </svg>
  );
}
